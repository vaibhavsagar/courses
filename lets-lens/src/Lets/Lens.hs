...
fmapT ::
  ...
  -> t b
fmapT f t =
  getIdentity $ traverse (Identity . f) t
...
sets ::
  ((a -> b) -> s -> t)
  -> Set s t a b
sets unTrav f = Identity . unTrav (getIdentity . f)

mapped ::
  Functor f =>
  Set (f a) (f b) a b
mapped f fa = Identity $ (getIdentity . f) <$> fa
...
set ::
  ...
  -> t
set st s b =
  getIdentity $ st (Identity . const b) s
...
_Right =
  dimap (either (Right . Left) Left) (either (fmap Right) pure) . left
...
prism ::
  ...
  -> Prism s t a b
prism to fro =
  dimap fro (either pure (fmap to)) . right
...
mapL k f m = (\v -> Map.alter (const v) k m) <$> f (Map.lookup k m)
...
setL k f m = (\a -> bool Set.delete Set.insert a k m) <$> f (Set.member k m)
...
choice  l r f = either (fmap Left . l f) (fmap Right . r f)
...
modifyIntAndLengthEven ::
  IntAnd [a]
  -> IntAnd Bool
modifyIntAndLengthEven (IntAnd n ls) = IntAnd n (even $ length ls)
...
traverseLocality f (Locality a b c) = Locality <$> f a <*> f b <*> f c
