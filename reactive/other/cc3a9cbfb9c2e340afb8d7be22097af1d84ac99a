...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe { value =>
      value match {
        case Success(sugs: List[String]) => suggestionList.listData = sugs
        case Failure(exception)          => status.text = exception.getMessage
      }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.filter { click =>
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.observeOn(eventScheduler) concatRecovered(wikiPageResponseStream)

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe { value =>
      value match {
        case Success(text: String) => editorpane.text = text
