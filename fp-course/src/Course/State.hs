...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f (State x) = State (\s -> let (a, s') = x s in (f a, s'))
...
findM f (h:.t)  = f h >>= \cond -> if cond then return (Full h) else findM f t
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat ls = eval (findM (\x -> do
  values <- get
  if x `S.member` values
    then return True
    else do put (x `S.insert` values)
            return False) ls) S.empty
...
isHappy ::
  Integer
  -> Bool
isHappy i = contains 1 (firstRepeat (produce sumSquareDigits i))

sumSquareDigits :: Integer -> Integer
sumSquareDigits n = toInteger $ sum (map ((P.^2) . digitToInt) (show' n))
