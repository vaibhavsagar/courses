...
bindParser ::
  ...
  -> Parser a
  -> Parser b
bindParser f (P parser) = P (\input -> case parser input of
  Result input' a -> let (P parser') = f a in parser' input'
...
(>>>) ::
  ...
  -> Parser b
  -> Parser b
(>>>) parserA parserB = parserA `flbindParser` const parserB
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) parserA parserB = P (\input -> case parse parserA input of
  r@(Result _ _)  -> r
  (ErrorResult _) -> parse parserB input)
...
list1 ::
  Parser a
  -> Parser (List a)
list1 parser = parser `flbindParser` (\a -> mapParser (a:.) (list parser))
...
natural ::
  Parser Int
natural = list1 digit `flbindParser` (\r -> case read r of
...
sequenceParser =
  foldRight (\a b -> a `flbindParser` (\a' -> mapParser (a':.) b)) (valueParser Nil)
...
firstNameParser ::
  Parser Chars
firstNameParser = upper `flbindParser` (\r -> mapParser (r:.) (list lower))
...
surnameParser ::
  Parser Chars
surnameParser = upper            `flbindParser` \up ->
                thisMany 5 lower `flbindParser` \manyLower ->
                list lower       `flbindParser` \rest ->
                valueParser $ up :. (manyLower ++ rest)
...
personParser ::
  Parser Person
personParser =
  ageParser                   `flbindParser` \parsedAge ->
  spaces1 >>> firstNameParser `flbindParser` \fName     ->
  spaces1 >>> surnameParser   `flbindParser` \sName     ->
  spaces1 >>> smokerParser    `flbindParser` \smokes    ->
  spaces1 >>> phoneParser     `flbindParser` \phoneNo   ->
  valueParser $ Person parsedAge fName sName smokes phoneNo
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) pF pA = pF `flbindParser` (`mapParser` pA)
