...
option ::
  ...
  -> Parser a
  -> Parser a
option value parser = parser ||| return value
...
hex ::
  Parser Char
hex = do
  value <- readHex <$> replicateA 4 (satisfy isHexDigit)
  case value of
    Full n -> return $ chr n
    Empty  -> failed
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll conds = let p c = all ($ c) conds in satisfy p
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny conds = let p c = any ($ c) conds in satisfy p
